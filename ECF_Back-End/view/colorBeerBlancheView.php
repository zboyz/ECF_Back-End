...
?>
    ...
    <main class="container mt-5">
        <h2 class="text-center mb-5 text-uppercase fw-bold text-white">Nos Blanches</h2>
                ...
                <nav class="d-flex justify-content-center my-3">
                    <ul class="pagination">
                        ...
                        <li class="page-item <?= ($currentPage == 1) ? "disabled" : "" ?>">
                            <a href="../controller/colorBeerBlanche.php?page=<?= $currentPage - 1 ?>" class="page-link"><i class="fa fa-chevron-left"></i></a>
                        </li>
                        <!-- Lien vers chacune des pages (activé si on se trouve sur la page correspondante) -->
                        <?php for($page = max(1, $currentPage - 3); $page <= min($currentPage + 3, $nbPages); $page++): ?>
                            <!-- Lien vers chacune des pages (activé si on se trouve sur la page correspondante) -->
                            <li class="page-item <?= ($currentPage == $page) ? "active" : "" ?>">
                                <a href="../controller/colorBeerBlanche.php?page=<?= $page ?>" class="page-link"><?= $page ?></a>
                            </li>
                        <?php endfor ?>
                        <!-- Lien vers la page suivante (désactivé si on se trouve sur la dernière page) -->
                        <li class="page-item <?= ($currentPage == $nbPages) ? "disabled" : "" ?>">
                            <a href="../controller/colorBeerBlanche.php?page=<?= $currentPage + 1 ?>" class="page-link"><i class="fa fa-chevron-right"></i></a>
                        </li>
                    </ul>
                </nav>
            </div>
